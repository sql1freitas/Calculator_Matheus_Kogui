...
def test_registro(client):
    response = client.post(reverse("login_registro"), {
        "form_type": "registro",
        "nome": "Teste",
...
def test_soma(client):
    ...
    assert b"5.0 soma 3.0 = 8.0" in response.content


@pytest.mark.django_db
def test_subtracao(client):
    Usuario = User.objects.create_user(email="sub@example.com", nome="Sub Teste", password="Senha123!")
    ...
    assert b"10.0 subtracao 4.0 = 6" in response.content
...
def test_multiplicacao(client):
    ...
    assert b"2.0 multiplicacao 6.0 = 12" in response.content
...
def test_divisao(client):
    ...
    assert b"10.0 divisao 2.0 = 5.0" in response.content
